// Code generated by ObjectBox; DO NOT EDIT.

package model

import (
	"github.com/google/flatbuffers/go"
	"github.com/objectbox/objectbox-go/objectbox"
	"github.com/objectbox/objectbox-go/objectbox/fbutils"
)

type table_EntityInfo struct {
	Id  objectbox.TypeId
	Uid uint64
}

var TableBinding = table_EntityInfo{
	Id:  1,
	Uid: 8927767662064279822,
}

// Table_ contains type-based Property helpers to facilitate some common operations such as Queries.
var Table_ = struct {
	Id   *objectbox.PropertyUint64
	Name *objectbox.PropertyString
	data *objectbox.PropertyByteVector
}{
	Id: &objectbox.PropertyUint64{
		Property: &objectbox.Property{
			Id: 1,
		},
	},
	Name: &objectbox.PropertyString{
		Property: &objectbox.Property{
			Id: 2,
		},
	},
	data: &objectbox.PropertyByteVector{
		Property: &objectbox.Property{
			Id: 3,
		},
	},
}

// GeneratorVersion is called by the ObjectBox to verify the compatibility of the generator used to generate this code
func (table_EntityInfo) GeneratorVersion() int {
	return 1
}

// AddToModel is called by the ObjectBox during model build
func (table_EntityInfo) AddToModel(model *objectbox.Model) {
	model.Entity("Table", 1, 8927767662064279822)
	model.Property("Id", objectbox.PropertyType_Long, 1, 2179264512159662000)
	model.PropertyFlags(objectbox.PropertyFlags_ID)
	model.Property("Name", objectbox.PropertyType_String, 2, 9048017577389059812)
	model.Property("data", objectbox.PropertyType_ByteVector, 3, 885629812034126046)
	model.EntityLastPropertyId(3, 885629812034126046)
}

// GetId is called by the ObjectBox during Put operations to check for existing ID on an object
func (table_EntityInfo) GetId(object interface{}) (uint64, error) {
	return object.(*Table).Id, nil
}

// SetId is called by the ObjectBox during Put to update an ID on an object that has just been inserted
func (table_EntityInfo) SetId(object interface{}, id uint64) error {
	object.(*Table).Id = id
	return nil
}

// Flatten is called by the ObjectBox to transform an object to a FlatBuffer
func (table_EntityInfo) Flatten(object interface{}, fbb *flatbuffers.Builder, id uint64) {
	obj := object.(*Table)
	var offsetName = fbutils.CreateStringOffset(fbb, obj.Name)
	var offsetdata = fbutils.CreateByteVectorOffset(fbb, obj.data)

	// build the FlatBuffers object
	fbb.StartObject(3)
	fbutils.SetUint64Slot(fbb, 0, id)
	fbutils.SetUOffsetTSlot(fbb, 1, offsetName)
	fbutils.SetUOffsetTSlot(fbb, 2, offsetdata)
}

// ToObject is called by the ObjectBox to load an object from a FlatBuffer
func (table_EntityInfo) ToObject(bytes []byte) interface{} {
	table := &flatbuffers.Table{
		Bytes: bytes,
		Pos:   flatbuffers.GetUOffsetT(bytes),
	}

	return &Table{
		Id:   table.GetUint64Slot(4, 0),
		Name: fbutils.GetStringSlot(table, 6),
		data: fbutils.GetByteVectorSlot(table, 8),
	}
}

// MakeSlice is called by the ObjectBox to construct a new slice to hold the read objects
func (table_EntityInfo) MakeSlice(capacity int) interface{} {
	return make([]*Table, 0, capacity)
}

// AppendToSlice is called by the ObjectBox to fill the slice of the read objects
func (table_EntityInfo) AppendToSlice(slice interface{}, object interface{}) interface{} {
	return append(slice.([]*Table), object.(*Table))
}

// Box provides CRUD access to Table objects
type TableBox struct {
	*objectbox.Box
}

// BoxForTable opens a box of Table objects
func BoxForTable(ob *objectbox.ObjectBox) *TableBox {
	return &TableBox{
		Box: ob.InternalBox(1),
	}
}

// Put synchronously inserts/updates a single object.
// In case the Id is not specified, it would be assigned automatically (auto-increment).
// When inserting, the Table.Id property on the passed object will be assigned the new ID as well.
func (box *TableBox) Put(object *Table) (uint64, error) {
	return box.Box.Put(object)
}

// PutAsync asynchronously inserts/updates a single object.
// When inserting, the Table.Id property on the passed object will be assigned the new ID as well.
//
// It's executed on a separate internal thread for better performance.
//
// There are two main use cases:
//
// 1) "Put & Forget:" you gain faster puts as you don't have to wait for the transaction to finish.
//
// 2) Many small transactions: if your write load is typically a lot of individual puts that happen in parallel,
// this will merge small transactions into bigger ones. This results in a significant gain in overall throughput.
//
//
// In situations with (extremely) high async load, this method may be throttled (~1ms) or delayed (<1s).
// In the unlikely event that the object could not be enqueued after delaying, an error will be returned.
//
// Note that this method does not give you hard durability guarantees like the synchronous Put provides.
// There is a small time window (typically 3 ms) in which the data may not have been committed durably yet.
func (box *TableBox) PutAsync(object *Table) (uint64, error) {
	return box.Box.PutAsync(object)
}

// PutAll inserts multiple objects in single transaction.
// In case Ids are not set on the objects, they would be assigned automatically (auto-increment).
//
// Returns: IDs of the put objects (in the same order).
// When inserting, the Table.Id property on the objects in the slice will be assigned the new IDs as well.
//
// Note: In case an error occurs during the transaction, some of the objects may already have the Table.Id assigned
// even though the transaction has been rolled back and the objects are not stored under those IDs.
//
// Note: The slice may be empty or even nil; in both cases, an empty IDs slice and no error is returned.
func (box *TableBox) PutAll(objects []*Table) ([]uint64, error) {
	return box.Box.PutAll(objects)
}

// Get reads a single object.
//
// Returns nil (and no error) in case the object with the given ID doesn't exist.
func (box *TableBox) Get(id uint64) (*Table, error) {
	object, err := box.Box.Get(id)
	if err != nil {
		return nil, err
	} else if object == nil {
		return nil, nil
	}
	return object.(*Table), nil
}

// Get reads all stored objects
func (box *TableBox) GetAll() ([]*Table, error) {
	objects, err := box.Box.GetAll()
	if err != nil {
		return nil, err
	}
	return objects.([]*Table), nil
}

// Remove deletes a single object
func (box *TableBox) Remove(object *Table) (err error) {
	return box.Box.Remove(object.Id)
}

// Creates a query with the given conditions. Use the fields of the Table_ struct to create conditions.
// Keep the *TableQuery if you intend to execute the query multiple times.
// Note: this function panics if you try to create illegal queries; e.g. use properties of an alien type.
// This is typically a programming error. Use QueryOrError instead if you want the explicit error check.
func (box *TableBox) Query(conditions ...objectbox.Condition) *TableQuery {
	return &TableQuery{
		box.Box.Query(conditions...),
	}
}

// Creates a query with the given conditions. Use the fields of the Table_ struct to create conditions.
// Keep the *TableQuery if you intend to execute the query multiple times.
func (box *TableBox) QueryOrError(conditions ...objectbox.Condition) (*TableQuery, error) {
	if query, err := box.Box.QueryOrError(conditions...); err != nil {
		return nil, err
	} else {
		return &TableQuery{query}, nil
	}
}

// Query provides a way to search stored objects
//
// For example, you can find all Table which Id is either 42 or 47:
// 		box.Query(Table_.Id.In(42, 47)).Find()
type TableQuery struct {
	*objectbox.Query
}

// Find returns all objects matching the query
func (query *TableQuery) Find() ([]*Table, error) {
	objects, err := query.Query.Find()
	if err != nil {
		return nil, err
	}
	return objects.([]*Table), nil
}
